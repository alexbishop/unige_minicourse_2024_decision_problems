...
\DontPrintSemicolon
\newcommand{\EOL}{\,\textbf{;}\;}
\newcommand{\EOLComment}[1]{\,\textbf{;}\tcp*[l]{#1}}
...
\begin{algorithm}[H]
  ...
  \tcp{The following helper function\\
...
\begin{algorithm}[H]
	...
	\MainFn{\sqrtFn{n$\colon\mathbb N$} $\mathbb N$} {
		...
		% \tcp{we may only reach this point if guess is the correct output}
		\Return $\mathrm{guess}$\EOL
	}
...
\algorithmstyle{plainruled}
	...
	\SetKwFunction{exampleFn}{functionName}
...
\begin{algorithm}[H]
  ...
  \tcp{the following will be used to capture the output of the method}
  \Assign{$a\colon \mathbb N$}{$0$}\EOL
  \Assign{$b\colon \mathbb N$}{$0$}\EOL
  \tcp{make the computation}
...
\begin{definition}\label{def:re-set}
  ...
  % In fact, it is not difficult to show that without loss of generality, we may assume that $ \mathrm{dom}(f) = S$.
\end{definition}
...
%
% In \cref{sec:solvable-simple}, we said that we could `run two algorithms in parallel'.
% We will now show that this is possible using simple modifications of algorithms as follows.
...
\begin{algorithm}[ht!]
  ...
  \MainFn{\parallelFn{$a_1\colon I_1$, $a_2\colon I_2$, $\ldots$, $a_k\colon I_k$} $O_1 \times O_2 \times \cdots O_\ell$} {
    \tcp{the following are used to store the output of $f'_1$ and $f'_2$}
    ...
    \tcp{the following is the current limit on the number of evaluations}
    \Assign{$\mathrm{currentLimit}\colon \mathbb N$}{$1$}\EOL

    \While{$\mathrm{true}$}{
      \tcp{see if limit is sufficient for $f_1'$}
...
\SetKwFunction{negateElementFn}{inverseOfElement}
  ...
  \tcp{computes the inverse an element encoded by $\varphi$}
